...
if len(line) != 0:    
    for x in range(len(line)-1):
        ...
        ffmpeg_extract_subclip(args.video, math.floor(int(line[x])/frame_count), math.floor(int(line[x])/frame_count) + 3, targetname=dir + f"/blood/{video_name}.mp4")
    ...
    if duration - math.floor(int(line[len(line)-1])/frame_count) > 3:
        ffmpeg_extract_subclip(args.video, math.floor(int(line[len(line)-1])/frame_count), math.floor(int(line[len(line)-1])/frame_count) + 3, targetname=dir + f"/blood/{video_name}.mp4")    
    else:
        ffmpeg_extract_subclip(args.video, math.floor(int(line[len(line)-1])/frame_count), duration, targetname=dir + f"/blood/{video_name}.mp4")  
