    ...
    return (
      <div className = "ob-con">
        ...
        <div className = "object1-img">
          ...
          <div className = "object-space"></div>
          <div className="object-text">
            <div className = "font-in">2단계 방식의 객체 탐지 알고리즘,</div> Faster RCNN 알고리즘 이름에
            ‘빠른(Faster)’이라는 단어가 포함되어 있지만 이는 단일 단계 방식보다
            빠른 처리가 된다는 뜻이 아닌 이전 버전이라 할 수 있는 RCNN
            알고리즘과 Fast RCNN 알고리즘 보다 빠르다는 것을 뜻한다. 각 관심
            영역(RoI; Region of Interest)에 대한 피쳐 추출의 계산을 공유하고
            딥러닝 기반의 RPN을 도입해 구현한다.
          </div>
        </div>
      </div>
    );
